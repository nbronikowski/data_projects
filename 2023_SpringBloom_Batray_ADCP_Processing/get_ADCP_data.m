...
timeVec = gdat.time;
uprofs_o = unique(gdat.prof_idx(rem(gdat.prof_idx,1)==0));
uprofs = 1:max(uprofs_o);
k = 1;
for i = 1:2:length(uprofs)-1
    idx1=find(gdat.prof_idx == uprofs(i) )  ; 
    idx2=find(gdat.prof_idx == uprofs(i)+1) ; 
    ...
    if ~isempty(idx1) && ~isempty(idx2)
        ...
        if deltaTime>5
            gdat.SegStartTime(k) = nanmin(timeVec(idx));
            gdat.SegEndTime(k) = nanmax(timeVec(idx));
            k = k + 1;
            %figure(1)
            %plot(timeVec(idx),-gdat.m_depth(idx),'.'); hold on
            %cla
        end
    else
    end
...
dat = parse_nortek_adcp('./AD2CP_data/Data.ad2cp');
