...
% paired by date and the thomalla correction for the daytime observations
    ...
    for k = 1:length(miss)
        ...
        if size(thomalla_correction(j-1).chl_night,2) == 1
            ...
            thomalla_correction(j).thomallacorrect = mean(thomalla_correction(j-1).chl_night,2,'omitnan')./mean(thomalla_correction(j-1).bbp_night,2,'omitnan').*thomalla_correction(j).bbp_day;
        end
    end
end