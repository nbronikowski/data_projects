...
%% We need a figure that takes all Argo and glider data for a select density range and then
...
% Get the fitted line
x_fit = linspace(285,310, 200); % Generating linear space for fit line
...
text(302, 285, txt); % Adjust the position as needed

% Labels and title (customize as needed)
xlabel('O^{Argo}_2 / \mumol kg^{-1}');
ylabel('O^{Glider}_2 / \mumol kg^{-1}');
title('Pearldiver vs Argo Oxygen in same \sigma_t bins (0.02 kg m^{-3})');

legend([h1; h2; h4 ;h3(1)],{'Data';'Robust Fit';'1:1 Fit';'95% FIT CI'},'Location','best')
grid on; formatplot

save_figure(gcf,['./plots/Pearldiver_vs_Argo_Oxygen'],[5.5 4],'.png','300')
...
formatplot
% legend([h1 h2],{'glider','ARGO'},'Location','NW')
title('(d)')
