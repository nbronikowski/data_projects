...
t1 = datenum(2022,01,16); t2 = datenum(2022,05,15); 
id = dat.pressure<10;
...
startDate = datenum('1-January-2022');
...
[S,idx1]=deleteAlmostEmptyColumns(dat.gridded.salinity,dat.gridded.pressure_grid);
S  = interp2(dat.gridded.timeg(idx1),dat.gridded.pressure_grid(1:1010)',...
    S(1:1010,:),Xq,Yq);
T  = interp2(dat.gridded.timeg(idx1),dat.gridded.pressure_grid(1:1010)',...
    dat.gridded.temperature(1:1010,idx1),Xq,Yq);
...
for i = 1:length(Xq(1,:))
    ...
    idnan = isnan(v);
    zn = z(~idnan);
    z_start = nanmin(zn); z_end = nanmax(zn);
    z_mld  = MLD(i);
    if isnan(z_mld)
   ...
   if isnan(z_mld)
    ...
    if (length(v(~idnan))>2) && (z_end-z_start>200) && ...
            z_start<100 && z_end>500 && ~isnan(z_mld)
...
idt1 = FAS_1hr.time_3d>datenum(2022,01,16) & FAS_1hr.time_3d<datenum(2022,01,31);
idt2 = FAS_1hr.time_3d>datenum(2022,04,01) & FAS_1hr.time_3d<datenum(2022,05,15);
...
legend([h1 h2],{'1-hr','3-d'},'Location','West')

subtitle(t,'Glider Sunfish Flux Computations')


save_figure(gcf,'./plots/sunfish_O2_flux_comparison',[7.5 5],'.png','300')
