...
% hold on; 
% topoplot([-60 -49 52 59],[],-3:1:-1,'2k-')

% Distance to HB
dist21000m = lldistkm([dat.latitude(:),dat.longitude(:)],[55.667,-53.9167]);

%% Gridd data to 1 hr and load era5 data
% MEDIAN RESOLUTION 4 hrs 


idt = dat.gridded.timeg>datenum(2020,01,01) & dat.gridded.timeg>datenum(2020,01,31);
...
t1 = datenum(2020,01,01); t2 = datenum(2020,06,01); 
...
t3 = datenum(2020,02,01); t4 = datenum(2020,05,01); 
...
t3 = datenum(2020,02,01); t4 = datenum(2020,05,31); 
...
h3=plot(x_fit, conf_int, 'r:'); % 95% prediction bounds
txt = sprintf('Slope: %.3f\nIntercept: %.3f\nR^2: %.3f', coeffs(1), coeffs(2), gof.rsquare);
text(140, 10, txt); % Adjust the position as needed
ylabel('\Delta O_2 / \mumol kg^{-1}')
xlabel('Track Distance to z=1000m / km')
title('(b)')
legend([h2 h4],{'Robust Fit','1:1 fit'},'Location','SW');
subtitle(tt,'Glider Parldiver Oxygen Anomaly to January at 900 m')

% save_figure(gcf,'./plots/pearldiver_oxygen_vs_distance',[7.5 4],'.png','300')
...
% subtitle(t,'pearldiver Depth Averaged Currents, Ekman, Oxygen and MLD')
