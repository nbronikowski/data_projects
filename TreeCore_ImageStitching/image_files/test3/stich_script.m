clear; clear all;
imageDir = fullfile('*.jpg');
buildingScene = imageDatastore(imageDir);
...
% Next, compute the average X limits for each transformation and find the
% image that is in the center. Only the X limits are used here because the
% scene is known to be horizontal. If another set of images are used, both
% the X and Y limits may need to be used to find the center image.
...
for i = 1:numel(tforms)    
    tforms(i).T = double(Tinv.T * tforms(i).T);
end
